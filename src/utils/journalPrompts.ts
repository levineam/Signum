...
export function getNextPrompt(): string {
  ...
  if (typeof window === 'undefined') return JOURNAL_PROMPTS[0];

  const lastIndex = parseInt(localStorage.getItem('journal-prompt-index') || '-1');
  const nextIndex = (lastIndex + 1) % JOURNAL_PROMPTS.length;
  localStorage.setItem('journal-prompt-index', nextIndex.toString());
...
export function getCurrentPrompt(): string {
  ...
  const currentIndex = parseInt(localStorage.getItem('journal-prompt-index') || '0');
  return JOURNAL_PROMPTS[currentIndex];
}

export function dismissPromptForSession(): void {
  if (typeof window === 'undefined') return;
  sessionStorage.setItem('prompt-dismissed', 'true');
}

export function isPromptDismissedForSession(): boolean {
  if (typeof window === 'undefined') return false;
  return sessionStorage.getItem('prompt-dismissed') === 'true';
}

export function resetPromptRotation(): void {
  localStorage.removeItem('journal-prompt-index');
}