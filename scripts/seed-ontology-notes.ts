/**
 * Seed script for adding 20 sample notes designed to test ontological extraction.
 * These notes contain rich semantic content with themes, entities, relationships,
 * and values that should be detected by the AI ontology system.
 *
 * Run this in the browser console on localhost:3000/notes:
 * 1. Copy this entire script
 * 2. Paste and run in browser console
 * 3. Refresh the page to see the notes
 */
...
function seedNotes() {
  try {
    ...
    const newNotes: Note[] = ontologyTestNotes.map((noteData, index) => ({
      ...noteData,
