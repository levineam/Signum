...
4) **Reasonable defaults**: Provide 1–2 sample projects per existing goal, each with 1–2 tasks; leave unassigned column empty unless explicitly used. Keep titles concise to avoid wrapping bloat.  
5) **Visibility & counts**: Column counts reflect seeded items; unassigned remains at 0 unless user moves items. Visually indicate seeds (e.g., subtle “Sample” chip or helper text) while keeping them fully editable.  
